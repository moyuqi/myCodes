...
function addFileId(fileId,serverData){
	var row = document.getElementById(fileId);
    var oServerData = eval('(' + serverData + ')');
    var sInputCheck =  "<input type='checkbox' name='FJ.attach' value='"+oServerData.data.id+"'checked='checked'/>";
    row.cells[3].innerHTML =  sInputCheck ;
...
function addReadyFileInfo(fileid,fileName,message,status){
	//用表格显示
	var swfuploadInfoTable = document.getElementById("swfuploadInfoTable");
	$("#divSwfuploadInfoContainer").css("display","block");
	//获取浏览器的类型
	...
	switch(oType){
	    case "ie":
	    	row = swfuploadInfoTable.insertRow();
	    	row.id = fileid;
	    	col1 = row.insertCell();
	    	col2 = row.insertCell();
	    	col3 = row.insertCell();
	        ...
	        break;
	    case "firefox":
	    	row = swfuploadInfoTable.insertRow(-1);
	    	row.id = fileid;
    ...
    if(status!=null&&status!=""){
        col2.innerHTML="<font color='red'>"+status+"</font>";
    }else{
        col2.innerHTML="";
    }
    col3.innerHTML = "<span class='swfu-filename'>"+fileName+"</span>";
    col1.style.width="80";
    col2.style.width="150";
    col3.style.width="150";
    col4.style.display="none";
}

function cancelUpload(){
	var swfuploadInfoTable = document.getElementById("swfuploadInfoTable");
	var rows = swfuploadInfoTable.rows;
	var ids = new Array();
	var row;
	if(rows==null){
		return false;
...
function deleteFile(fileId){
	...
	$.ajax({
		...
		success : function(data) { // 判断是否成功
			//处理被删除的节点
            swfuploadInfoTable.deleteRow(row.rowIndex);
...
function addImage(src) {
	...
	document.getElementById("divSwfuploadInfoContainer").appendChild(newImg);
	if (newImg.filters) {
		try {
